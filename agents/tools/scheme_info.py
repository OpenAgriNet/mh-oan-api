...
# Load scheme categorization from JSON (single source of truth)
_scheme_list = json.load(open('assets/scheme_list.json', 'r', encoding='utf-8'))
STATE_SCHEMES = {s["scheme_code"] for s in _scheme_list if s.get("type") == "state"}
CENTRAL_SCHEMES = {s["scheme_code"] for s in _scheme_list if s.get("type") == "central"}

_scheme_codes_set = {s["scheme_code"] for s in _scheme_list}
...
def _validate_scheme_code(scheme_code: str) -> bool:
    ...
    """
    return scheme_code in _scheme_codes_set
...
async def get_scheme_codes() -> str:
    ...
    scheme_lookup = {scheme['scheme_code']: scheme for scheme in _scheme_list}
